...
describe("GET /users/:username", function () {
  ...
  test("unauth for the wrong user", async function () {
    const resp = await request(app)
      .get(`/users/u2`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("PATCH /users/:username", () => {
  ...
  test("works for non-admin user on themselves: set new password", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
    ...
    const isSuccessful = await User.authenticate("u1", "newer-password");
    expect(isSuccessful).toBeTruthy();
  });
